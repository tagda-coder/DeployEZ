    ...
    <div className="flex flex-col gap-8 w-full animate-in slide-in-from-bottom-4 fade-in duration-500 pb-10">
      ...
      <div className="mt-4 flex flex-col relative z-10 w-full max-w-5xl">
        <div className="bg-(--card-bg) border border-(--card-border) rounded-2xl overflow-hidden shadow-sm">
           ...
           <div className="flex items-center justify-between p-5 border-b border-(--card-border) bg-(--bg-base)">
             ...
             <div className="flex items-center gap-3">
               ...
               <button className="flex items-center gap-2 px-3 py-1.5 text-sm bg-(--btn-primary-bg) text-(--btn-primary-text) hover:bg-(--btn-primary-bg-hover) hover:text-(--btn-primary-text-hover) rounded-md font-medium transition-all duration-300 shadow-sm ml-4 border border-(--card-border)">
           ...
           <div className="flex flex-col p-5 gap-4 bg-(--card-bg)">
              {envs.map((env, i) => (
                <div key={i} className="flex flex-col md:flex-row md:items-center gap-4 group">
                  ...
                  <button className="p-2.5 text-(--text-muted) hover:text-(--status-error) hover:bg-(--status-error-bg) rounded-lg transition-all shrink-0">
