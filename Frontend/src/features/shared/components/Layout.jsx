...
// Derives initials from username or email
    ...
    <div className="flex flex-col md:flex-row h-screen w-full bg-(--bg-base) text-(--text-base) overflow-hidden relative">
      ...
      <div className="md:hidden flex items-center justify-between p-4 border-b border-(--sidebar-border) bg-(--sidebar-bg) z-30 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xl font-bold tracking-wide text-(--text-primary)">DeployEZ</span>
        </div>
          ...
          {showUserMenu && (
            ...
            >
              <div className="px-3 py-2 border-b border-(--card-border) mb-2">
                ...
                {displayEmail && <p className="text-xs text-(--text-muted) truncate">{displayEmail}</p>}
              </div>
      ...
      >
        <div className="flex flex-col h-full overflow-hidden">
          <div className="flex items-center justify-between mb-10 px-5 whitespace-nowrap overflow-hidden transition-all duration-300">
            <div className="flex items-center gap-3">
              <button
            ...
            <button className="md:hidden p-2 text-(--text-muted) hover:text-(--text-primary) hover:bg-(--card-bg) rounded-xl" onClick={() => setIsMobileOpen(false)}>
          ...
          <nav className="flex flex-col gap-2 px-4">
            {navItems.map((item) => (
              ...
              >
                ...
                <span className={`transition-opacity duration-300 ${isCollapsed && !isMobileOpen ? "opacity-0" : "opacity-100"}`}>
                  {item.name}
                </span>
              </NavLink>
            ))}
        ...
        <div className="px-4 mt-auto hidden md:block">
          <div className="pt-6 border-t border-(--sidebar-border) relative">
            ...
            >
              ...
              <div className={`flex flex-col items-start transition-opacity duration-300 ${isCollapsed && !isMobileOpen ? "opacity-0 w-0" : "opacity-100"}`}>
                <span className="text-sm font-medium text-(--text-primary)">{displayName}</span>
                <span className="text-xs text-(--text-muted) truncate max-w-[140px]">{displayEmail}</span>
              </div>
            </button>
      ...
      <main className="flex-1 relative flex flex-col h-full overflow-hidden">
        <BackgroundPattern />
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 lg:p-12 relative z-10 w-full mx-auto max-w-400">
          <Outlet />
        </div>
      </main>
