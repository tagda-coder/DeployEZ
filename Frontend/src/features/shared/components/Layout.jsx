...
import { LayoutDashboard, BarChart2, CloudUpload, PanelLeftClose, PanelLeft, LogOut, Settings as SettingsIcon } from "lucide-react";
...
const Layout = () => {
  ...
  const navItems = [
    ...
    { name: "Deployments", path: "/deploy", icon: CloudUpload },
  ];
    ...
    <div className="flex h-screen w-full bg-(--bg-base) text-(--text-base) overflow-hidden">
      ...
      >
        <div className="flex flex-col h-full overflow-hidden">
          <div className={`flex items-center mb-10 px-5 whitespace-nowrap overflow-hidden transition-all duration-300 ${isCollapsed ? 'justify-center' : 'justify-between'}`}>
            {!isCollapsed && <span className="text-xl font-semibold tracking-wide text-(--text-primary) animate-in fade-in duration-300">DeployEZ</span>}
            <button 
